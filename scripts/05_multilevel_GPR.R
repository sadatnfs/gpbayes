    ...
    rm(list=ls())
    pacman::p_load(data.table, TMB, ggplot2, geostatsp, INLA, brinla, MASS, ar.matrix, mvtnorm, Matrix, sparseMVN)
    ...
    ## Random effects ##
      ...
      # Simulate based off of the sparse precision matrix
        simm = array(data=rmvn.sparse(1, rep(0, nrow(Q_all)), Cholesky(Matrix(Q_all, sparse = TRUE)), T), 
    ...
    ## Clean, compile and load model
      # system('rm ~/gpbayes/scripts/*.o ~/gpbayes/scripts/*.so'); TMB::compile('~/gpbayes/scripts/05_multilevel_GPR_model.cpp')
      dyn.load(dynlib("~/gpbayes/scripts/05_multilevel_GPR_model"))
    ...
    ### DATA ARRAYS ###
      ...
      ## Create list of data
        data_in <- list(X = array_x , Y = array_y,
    ...
    ########################################################################
      ...
      system.time(output_list[['opt']] <- nlminb(objective = output_list[['obj']]$fn, 
    ...
    ## Pull out precision matrix and mean 
      mu <- c(output_list[['FC_model']]$par.fixed, output_list[['FC_model']]$par.random)
      
    # Simulate draws
      samples = 100
      draws <- rmvnorm_prec(mu = mu*0, output_list[['FC_model']]$jointPrecision, samples)
      
      
    ## Separate out the draws
      parnames      <- c(names(output_list[['FC_model']]$par.fixed), names(output_list[['FC_model']]$par.random))
    ...
    ## Create quantiles of the draws
      quantile_Yhat <- draws_yhat[, as.list(c(mean(Ypred), quantile(Ypred, c(0.025, 0.975)))), by = c("L","A","Y")]
    ...
    ## Make some plots (for a given location, across time and age)
      ...
      ggplot() +
        ...
        ## Fitted
        geom_ribbon(data = quantile_Yhat[L == Lplot], aes(x = Y, ymin = lower, ymax = upper), alpha = 0.3, color = NA, fill = 'steelblue') +
    ...
    ## Plot for a given age, across time and location
      ...
      ggplot() +
        ...
        ## Fitted
        geom_ribbon(data = quantile_Yhat[A == Aplot], aes(x = Y, ymin = lower, ymax = upper), alpha = 0.3, color = NA, fill = 'steelblue') +
